...
#
# Usage:
#   ./deploy-rollback-sink.sh <instance_name> [--yes]
set -euo pipefail
...
usage() {
  echo "Usage: $0 <instance_name> [--yes]" >&2
  echo "  instance_name: matches instances/<instance_name>.env" >&2
  echo "  MySQL rollback connection details are read from instances/<instance_name>.env:" >&2
  echo "    ORIGINAL_MYSQL_HOST, ORIGINAL_MYSQL_PORT, ORIGINAL_MYSQL_USER," >&2
  echo "    ORIGINAL_MYSQL_PASSWORD, ORIGINAL_MYSQL_DBNAME" >&2
...
: "${ORIGINAL_MYSQL_HOST:?ORIGINAL_MYSQL_HOST is not set in ${INSTANCE_ENV_FILE}}"
: "${ORIGINAL_MYSQL_PORT:?ORIGINAL_MYSQL_PORT is not set in ${INSTANCE_ENV_FILE}}"
: "${ORIGINAL_MYSQL_USER:?ORIGINAL_MYSQL_USER is not set in ${INSTANCE_ENV_FILE}}"
: "${ORIGINAL_MYSQL_PASSWORD:?ORIGINAL_MYSQL_PASSWORD is not set in ${INSTANCE_ENV_FILE}}"
: "${ORIGINAL_MYSQL_DBNAME:?ORIGINAL_MYSQL_DBNAME is not set in ${INSTANCE_ENV_FILE}}"

MYSQL_HOST="$ORIGINAL_MYSQL_HOST"
MYSQL_PORT="$ORIGINAL_MYSQL_PORT"
MYSQL_USER="$ORIGINAL_MYSQL_USER"
MYSQL_PASSWORD="$ORIGINAL_MYSQL_PASSWORD"
MYSQL_DBNAME="$ORIGINAL_MYSQL_DBNAME"
...
PAYLOAD=$(jq -n \
  ...
  '{name: $name, config: ($config + {
      "topics.regex": "^cdc-\($inst)\\.(?!cdc\\.debezium_heartbeat$).*$",
      "connection.url": $url,
