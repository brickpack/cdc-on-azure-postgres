#!/usr/bin/env bash
# Full CDC pipeline status report (AKS).
...
#   ./aks/scripts/cdc-status.sh --instance billing --tables               # topic totals only (fast)
#   ./aks/scripts/cdc-status.sh --instance billing --tables --ops         # topic totals + INSERT/UPDATE/DELETE breakdown (slow)
#
# Postgres connection details (POSTGRES_HOST/PORT/USER/PASSWORD/DBNAME,
# SLOT_NAME) are not stored anywhere on AKS -- export them yourself before
# running this script, matching that instance's postgres: block in
# aks/values.local.yaml (slot name is cdc_<name> by convention).
#
...
#
# Docker Compose equivalent: docker/scripts/cdc-status.sh
set -uo pipefail
...
# Main
# ===========================================================================
echo -e "${BOLD}CDC Pipeline Status Report — $(date '+%Y-%m-%d %H:%M:%S %Z') (aks: ${NAMESPACE}/${INSTANCE})${NC}"
